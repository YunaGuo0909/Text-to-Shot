...
PYTHONPATH=. python scripts/preprocess_et_data.py \
    ...
    --output-root /transfer/stc-data-v9 \
    --require-person \
    --num-frames 48
...
PYTHONPATH=. python scripts/prepare_amass.py \
    ...
    --output-root /transfer/amass-stc-data-v9 \
    --num-frames 48
...
PYTHONPATH=. python scripts/prepare_humanml3d.py \
    ...
    --output-root /transfer/humanml3d-stc-data-v9 \
    --num-frames 48

echo ""
echo "=== Step 4: Merge all sources ==="
PYTHONPATH=. python scripts/merge_datasets.py \
    --sources /transfer/stc-data-v9 /transfer/amass-stc-data-v9 /transfer/humanml3d-stc-data-v9 \
    --output-root /transfer/merged-v9 \
    --person-dim 3 \
    --camera-dim 6
...
python scripts/diagnose_v6_issues.py --data-root /transfer/merged-v9 --max-samples 2000

echo ""
echo "=== DONE ==="
echo "Update v9.yaml data_root to /transfer/merged-v9"
echo "Update v9.yaml norm_stats_path to /transfer/merged-v9/norm_stats_v9.json"
