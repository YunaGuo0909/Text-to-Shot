...
def visualize_joint(person_traj, camera_traj, text, motion, save_path):
    ...
    # Person facing direction arrows (if sin/cos yaw is available, dim >= 5)
    if person_traj.shape[1] >= 5:
        person_arrow_len = scene_extent * 0.10
        for i in range(0, num_frames, arrow_interval):
            px, pz = person_traj[i, 0], person_traj[i, 2]
            yaw = np.arctan2(person_traj[i, 3], person_traj[i, 4])
    ...
    if person_traj.shape[1] >= 5:
        # Recover yaw from sin/cos for display
        yaw_deg = np.degrees(np.arctan2(person_traj[:, 3], person_traj[:, 4]))
        ax5.plot(t_axis, yaw_deg, color='#FF9F43', linewidth=1.5, label='yaw (deg)')
    ax5.set_title('Person Position' + (' + Yaw' if person_traj.shape[1] >= 5 else ''),
